...
export async function unwarn(ctx) {
  ...
  try {
    ...
    return { success: false, message: '⚠️ Ocurrio un error al quitar las advertencias.' }
  }
}
...
export async function warns(ctx) {
  ...
  try {
    ...
    const list = rows
      .map((r, i) => `${i + 1}. @${r.user_jid.split('@')[0]} - ${r.count} advertencia(s)`)
      .join('\n')
    ...
    return { success: false, message: '⚠️ Ocurrio un error al obtener la lista de advertencias.' }
  }
}
