...
export const BotStatus: React.FC = () => {
  ...
  const { data: qrCode, isLoading: qrLoading } = useQuery('botQR', apiService.getBotQR, {
    enabled: !(botStatus?.connected || botStatus?.isConnected),
    refetchInterval: !(botStatus?.connected || botStatus?.isConnected) ? 10000 : false, // Actualizar QR cada 10 segundos si no está conectado
  });
  ...
  return (
    <Box>
      <VStack spacing={6} align="stretch">
        ...
        <Flex align="center" justify="space-between">
          ...
          <HStack spacing={3}>
            <Button
              ...
              variant="outline"
              onClick={onQROpen}
              isDisabled={botStatus?.connected || botStatus?.isConnected}
            >
              Ver QR
            </Button>
            <Button
          ...
          <CardBody>
            <SimpleGrid columns={{ base: 1, md: 2, lg: 4 }} spacing={6}>
              <Stat>
                ...
                <StatHelpText>
                  {(botStatus?.connected || botStatus?.isConnected) ? 'Bot funcionando correctamente' : 'Bot no está conectado'}
                </StatHelpText>
              </Stat>
        ...
        {/* Alertas */}
        {!(botStatus?.connected || botStatus?.isConnected) && (
          <Card bg={cardBg} border="1px" borderColor={borderColor}>
            <CardBody>
