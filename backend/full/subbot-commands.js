...
export async function handleSerbot(usuario, grupo, fecha, args = []) {
  try {
    ...
    if (result.success) {
      ...
      response += `📱 *Código:* ${result.subbot.code}\n`;
      response += `🔗 *Tipo:* ${connectionType === 'qr' ? 'Código QR' : 'Código de Emparejamiento'}\n\n`;
      
      if (connectionType === 'qr') {
        response += `📱 *Instrucciones:*\n`;
        response += `1. Abre WhatsApp en tu teléfono\n`;
        response += `2. Ve a Configuración > Dispositivos Vinculados\n`;
        response += `3. Toca "Vincular un dispositivo"\n`;
        response += `4. Escanea el código QR que aparecerá\n\n`;
        response += `⏰ El código QR expira en 1 minuto\n`;
        response += `💡 Usa /missubbots para ver el estado`;
      } else {
        response += `🔢 *Código de Emparejamiento:* ${result.pairingCode}\n\n`;
        response += `📱 *Instrucciones:*\n`;
        response += `1. Abre WhatsApp en tu teléfono\n`;
        response += `2. Ve a Configuración > Dispositivos Vinculados\n`;
        response += `3. Toca "Vincular un dispositivo"\n`;
        response += `4. Selecciona "Vincular con número de teléfono"\n`;
        response += `5. Ingresa el código: ${result.pairingCode}\n\n`;
        response += `💡 Usa /missubbots para ver el estado`;
      }
...
export async function handleMisSubbots(usuario, grupo, fecha) {
  try {
    ...
    if (result.success) {
      ...
      response += `• /delsubbot <código> - Eliminar subbot\n`;
      response += `• /statusbot <código> - Ver estado detallado`;
...
export async function handleDelSubbot(usuario, grupo, fecha, args = []) {
  try {
    ...
    if (result.success) {
      ...
      return {
        success: false,
        message: `❌ *Error eliminando SubBot*\n\n${result.error}\n\n💡 Verifica el código con /missubbots`
      };
    }
  } catch (error) {
  ...
  try {
    ...
    if (result.success) {
      ...
      const statusText = {
        ...
        'waiting_pairing': 'Esperando código de emparejamiento',
      ...
      if (subbot.last_heartbeat) {
        response += `⏰ *Última actividad:* ${new Date(subbot.last_heartbeat).toLocaleString()}\n\n`;
      }
