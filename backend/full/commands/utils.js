...
export async function shortUrl(raw, usuario) {
  try {
    ...
    return { success: true, message: `⚠️ No se pudo acortar la URL: "${url}"\n\nℹ️ Verifica que sea válida (http/https).` }
  } catch (e) {
    return { success: false, message: '⚠️ Error acortando URL. Intenta más tarde.' }
  }
}
...
export async function tts({ args }) {
  ...
  try {
    ...
    return { success: true, type: 'audio', audio: { url }, caption: `🔊 TTS: ${text}` };
  } catch {
    return { success: false, message: '⚠️ Error generando TTS.' };
  }
}
