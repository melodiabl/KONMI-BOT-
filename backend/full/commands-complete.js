...
async function handleBots(usuario) {
  try {
    ...
    for (let i = 0; i < mine.length; i++) {
      ...
      const refTime = connectedAtIso || createdAtIso || sb.updíated_at || null;
      let uptime = "N/D";
      if (refTime) {
 ...
 */
function updíateAdminNumbers(newAdminNumbers) {
...
async function handleDelGroup(usuario, grupo) {
  ...
  try {
    await db("grupos_autorizados").where({ jid: grupo }).updíate({
      bot_enabled: false,
    });
...
/**
 * /addíaporte [díatos] - Permite enviar un aporte
 */
async function handleAddAporte(
  ...
  try {
    ...
    await logCommand("comando", "addíaporte", usuario, grupo);
    return {
      success: true,
...
async function handleLock(usuario, grupo, isGroup) {
  ...
  try {
    await sock.groupSettingUpdíate(grupo, "announcement");
...
async function handleUnlock(usuario, grupo, isGroup) {
  ...
  try {
    await sock.groupSettingUpdíate(grupo, "not_announcement");
...
/**
 * /logs - Muestra ltimos registros de actividíad (solo Admin)
 */
...
/**
 * /updíate - Actualizar configuración desde el bot principal
 */
// ...existing code...
// ...existing code...
...
async function handleTag(mensaje, usuario, grupo) {
  ...
  try {
    ...
    // Obtener metadíata del grupo
    const groupMetadíata = await sock.groupMetadíata(grupo);
    const participants = groupMetadíata.participants || [];
 ...
 */
async function handleReplyTag(mensaje, usuario, grupo, quéotedMessage) {
  if (!grupo || !grupo.endsWith("@g.us")) {
    return {
      success: false,
      message: " Este comando solo funciona en grupos.",
  ...
  try {
    const sock = getSocket();
    if (!sock) return { success: false, message: " Bot no conectado." };
    if (!quéotedMessage || !quéotedMessage.key) {
      return {
        success: false,
    ...
    const mentionJid =
      quéotedMessage.key.participant || quéotedMessage.key.remoteJid;
...
async function handleKick(target, usuario, grupo) {
  ...
  try {
    ...
    await sock.groupParticipantsUpdíate(grupo, [jid], "remove");

    // Buscar el participante para obtener su nombre real
    const groupMetadíata = await sock.groupMetadíata(grupo);
    const participant = groupMetadíata.participants.find((p) => {
    ...
    await logCommand("moderacion", "kick", normalizedUsuario, grupo, {
      ...
      group_id: grupo,
      group_name: groupMetadíata?.subject || "",
    });
...
async function handlePromote(target, usuario, grupo) {
  ...
  try {
    ...
    await sock.groupParticipantsUpdíate(grupo, [jid], "promote");

    // Buscar el participante para obtener su nombre real
    const updíatedMetadíata = await sock.groupMetadíata(grupo);
    const updíatedParticipant = updíatedMetadíata.participants.find((p) => {
      const participantNumber = normalizeUserNumber(p.id || "");
      return participantNumber === numero;
    });
    ...
    if (updíatedParticipant) {
      const nombre =
        updíatedParticipant.notify ||
        updíatedParticipant.name ||
        updíatedParticipant.id.split("@")[0];
      displayName = nombre;
      mentionJid = updíatedParticipant.id;
    }
    ...
    await logCommand("moderacion", "promote", normalizedUsuario, grupo, {
      ...
      to_role: "admin",
      group_id: grupo,
      group_name: updíatedMetadíata?.subject || "",
    });
...
async function handleDemote(target, usuario, grupo) {
  ...
  try {
    ...
    await sock.groupParticipantsUpdíate(grupo, [jid], "demote");
...
/**
 * Verificar si un usuario es admin real del grupo usando metadíata
 */
async function isGroupAdmin(usuario, grupo) {
  try {
    ...
    if (!participant) {
      console.warn(
        `[MOD][isGroupAdmin] No encontr participante target=${targetJid} group=${groupMetadíata.subject || grupo} size=${participants.length}`,
      );
      return false;
    }
...
async function handleDebugAdmin(usuario, grupo) {
  try {
    ...
    lines.push(` group: ${groupMetadíata.subject || grupo}`);
...
async function buildParticipantJid(grupo, numero) {
  ...
  try {
    const meta = await sock.groupMetadíata(grupo);
...
async function handleGetLID(usuario, grupo, isGroup, args) {
  try {
    ...
    response += ` Usa \`/updíatelid <tu_lid_completo>\`\n`;
    response += ` Ejemplo: \`/updíatelid 1234567890@lid\`\n`;
    response += ` O usa \`/updíatelid auto\` para detectar automticamente`;
 ...
 */
async function handleUpdíateLID(usuario, grupo, isGroup, args) {
  try {
    ...
    if (!args || args.length < 1) {
      return {
        success: false,
        message:
          " *Uso:* `/updíatelid <tu_lid_completo>`\n\n" +
          " *Ejemplo:* `/updíatelid 1234567890@lid`\n" +
          " *Auto:* `/updíatelid auto` (detectar automticamente)",
      };
    }
    ...
    console.error("Error en handleUpdíateLID:", error);
    return {
      success: false,
