...
function buildSubbotSummary(rows) {
  ...
  rows.forEach((row) => {
    ...
    if (!item) return;
    if (item.type === 'qr') summary.qr += 1; else summary.code += 1;
    if (item.isOnline || item.status === 'connected') summary.connected += 1;
    else if (['pending', 'waiting_scan', 'waiting_pairing'].includes(item.status)) summary.pending += 1;
...
class SubbotService {
  constructor() {
    ...
    this.inactiveTimeout = parseInt(process.env.SUBBOT_TIMEOUT) || 30 * 60 * 1000;
    this.cleanupInterval = parseInt(process.env.CLEANUP_INTERVAL) || 5 * 60 * 1000;
  ...
  startCleanupService() {
    const interval = setInterval(() => this.intelligentCleanup(), this.cleanupInterval);
  ...
  try {
    ...
    sock.ev.on('messages.upsert', async (upsert) => {
      try {
        ...
        console.error(`Error procesando mensaje en subbot ${subbotCode}:`, error);
      }
    });
  } catch (error) {
    console.error(`Error configurando handlers para subbot ${subbotCode}:`, error);
  }
}
...
export async function createSubbot(userPhone, userName, connectionType = 'qr') {
  ...
  await db('subbots').insert({
    ...
    status: 'pending',
    settings: JSON.stringify({ autoReply: false, allowGroups: true, maxMessages: 1000 }),
  });
  ...
  emitSubbotEvent('launch', { code: subbotCode, subbot: formatSubbotRow(subbot) });
  return {
    success: true,
    connectionType,
...
export async function getSubbotStatus(subbotCode) {
  const subbot = await db('subbots').where({ code: subbotCode }).first();
...
export async function getSubbotAccessData(subbotCode, userPhone) {
  ...
  if (userPhone) {
    ...
    if (subbot.user_phone && normalized && subbot.user_phone !== normalized) {
      return { success: false, error: 'Subbot no autorizado para este usuario' };
    }
  }
  const qrData = subbot.qr_code ? subbot.qr_code.replace(/^data:image\/png;base64,/, '') : null;
  return {
    success: true,
    qr: qrData,
